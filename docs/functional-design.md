...
Showdown データと Pokémon Champions の差分（技威力・種族値変更など）をポケモンWiki（https://wiki.pokemonwiki.com/wiki/Pok%C3%A9mon_Champions）で調査し、`tools/PokelensMasterDataBuilder/Patches/champions-patch.json` に静的パッチとして管理する。C# ツールはShowdownデータ変換後にパッチを上書き適用する。
...
[Step 4] 最終出力生成 → data/*.json  （条件付き）
  ※ pokedex.json 生成時に以下を適用する:
    ...
    - メガストーン不要メガの特例ネスト (機能 7): items.ts に `megaStone` を持たない例外的メガフォーム (現状はメガレックウザのみ) を `MergeConverter` 内の `ItemlessMegas` ハードコードマップ経由で親エントリの `megaForms[]` にネストし、`item: null` を設定する。トップレベルキー削除と孤立メガ除去の対象から除外。将来「持ち物不要メガ」が 2 件以上に増えた場合は `tools/PokelensMasterDataBuilder/Patches/mega-itemless-patch.json` への昇格を検討する
...
Showdown の `items.js` / `abilities.js` は補正値をJavaScript関数で記述しているためC#から直接パースできない。補正値は `tools/PokelensMasterDataBuilder/Patches/items-modifiers.json` / `tools/PokelensMasterDataBuilder/Patches/abilities-modifiers.json` に手書きで管理し、Step4でMergeConverterが日本語名（`pokeapi-translations.json` 参照）と結合して `data/items.json` / `data/abilities.json` を生成する。新規アイテム・特性の追加やバランス変更時はこれらのJSONファイルを直接更新する。
...
> 上記は実装の代表抜粋。完全なエントリ群と「最大火力指向のため発動済みとして常時補正をかける」ポリシーの全文は `tools/PokelensMasterDataBuilder/Patches/abilities-modifiers.json` の `_comment_policy` を参照（プロジェクト名は機能 14 でリネーム済み。本書中の `PokelensMasterDataBuilder` は旧 `PokelensTools` と同義）。

---

## C# パーティ編集 GUI ツール（機能 14）

### 役割と位置付け

`data/party.json` を GUI 経由で編集するためのデスクトップアプリケーション。手動 JSON 編集の手間を解消し、対戦準備フローでの自分パーティ登録を支援する。フロントエンド（pokelens 本体）と並行して動作する独立ツールであり、フロントエンドとの連携は `data/party.json` ファイル経由のみ（GUI 保存後にユーザーが手動でブラウザをリロードする）。

### プロジェクト構成（`tools/` 配下）

| プロジェクト | 種別 | 役割 |
|---|---|---|
| `PokelensCore` | クラスライブラリ | サジェスト（`NameSearch` C# 版）・マスターデータ読み込み（`MasterDataReader`）・共通型定義（`PartyDocument`, `PokemonEntryModel` 等）。GUI フレームワーク非依存（Avalonia 等を参照しない） |
| `PokelensMasterDataBuilder` | コンソールアプリ | 旧 `PokelensTools` のリネーム。Showdown / PokéAPI からマスターデータを生成するパイプライン。`PokelensCore` を参照 |
| `PokelensPartyEditor` | Avalonia デスクトップアプリ | 機能 14 の本体。`PokelensCore` を参照。MVVM 構造（`Views/` / `ViewModels/` / `Services/` / `Models/`） |

依存方向: `PokelensMasterDataBuilder` → `PokelensCore` ← `PokelensPartyEditor`（Core は他に依存しない）。

### 技術スタック（機能 14 部分）

| 分類 | 技術 | 選定理由 |
|---|---|---|
| GUI フレームワーク | Avalonia UI 11.x | 開発環境（WSL2 Linux dev container）でビルド・実行可能なクロスプラットフォーム。Windows/Mac/Linux で同一コード |
| MVVM 基盤 | `CommunityToolkit.Mvvm` | `ObservableObject` / `RelayCommand` 等のボイラープレート削減 |
| GUI アーキテクチャ | MVVM | View（XAML）/ ViewModel / Model の 3 層。ViewModel 層を xUnit でユニットテスト可能にする |
| 依存性注入 | 不採用（手動 `new`） | 個人ローカル使用で規模が小さく、DI コンテナの導入コストに見合わない |

### アーキテクチャ（MVVM + Services）

```
┌─────────────────────────────────────────────────────────────┐
│ View 層 (Avalonia XAML)                                       │
│   MainWindow.axaml         — 6 匹分のグリッド + ロード/セーブ │
│   PokemonEntryView.axaml   — 1 匹分の入力欄                   │
│   SuggestPopup.axaml       — テキストボックスに紐づくサジェスト│
└────────────────────────────┬─────────────────────────────────┘
                             │ Binding
┌────────────────────────────▼─────────────────────────────────┐
│ ViewModel 層 (xUnit でテスト可能)                              │
│   MainWindowViewModel        — 全体状態（6 匹・ダーティフラグ）│
│   PokemonEntryViewModel × 6  — 各エントリの入力値・サジェスト  │
│   ValidationError            — バリデーション結果              │
└────────────────────────────┬─────────────────────────────────┘
                             │ 呼び出し
┌────────────────────────────▼─────────────────────────────────┐
│ Services 層 (PokelensPartyEditor 内・GUI 非依存)              │
│   PartyFileService    — data/party.json の読み書き             │
│   MasterDataService   — pokedex/abilities/items/moves をロード │
│   SuggestService      — PokelensCore.NameSearch を委譲呼び出し │
└────────────────────────────┬─────────────────────────────────┘
                             │ 参照
┌────────────────────────────▼─────────────────────────────────┐
│ PokelensCore (クラスライブラリ)                                │
│   NameSearch                — 正規化・前方一致（JS 版の C# 移植）│
│   MasterDataReader          — JSON → 強い型へのデシリアライズ │
│   Models (PartyDocument 等)  — party.json と 1:1 の型           │
└─────────────────────────────────────────────────────────────┘
```

### データモデル（C# 型・概要）

`PokelensCore.Models` 配下に定義する強い型:

| 型 | 役割 |
|---|---|
| `PartyDocument` | `data/party.json` のルート（`Party: PokemonEntryModel[]`） |
| `PokemonEntryModel` | 1 匹分（`Species`, `Ability`, `Item`, `Nature`, `AbilityPoints`, `Moves: MoveEntryModel[]`） |
| `AbilityPointsModel` | HP/Atk/Def/SpA/SpD/Spe の 6 値（各 0〜32） |
| `MoveEntryModel` | 1 技分（`Name`） |
| `MasterDataSnapshot` | `pokedex.json` / `abilities.json` / `items.json` / `moves.json` / `natures.json` のロード結果を束ねる |
| `ValidationError` | セーブ前バリデーション結果（フィールド・エラー種別・メッセージ） |

`PartyDocument` は `UserParty` エンティティ（後出）と JSON 上 1:1 対応。フロントエンドの JS と GUI の両方が `party.json` を真実源として共有する。

### 主要シーケンス

**ロード処理**:
1. 「ロード」ボタン押下 → `PartyFileService.LoadAsync()` を呼ぶ
2. ファイル不在 → エラーダイアログ「`data/party.json` が見つかりません」を表示し、画面状態は変更しない
3. JSON パース失敗 → エラーダイアログ「JSON 形式が不正です: 〈例外メッセージ〉」を表示し、画面状態は変更しない
4. 型不正（例: `species: 0`）→ エラーダイアログ「不正な型が含まれます」を表示し、画面状態は変更しない
5. 既知スキーマ外のキー・値（例: `"hoge": "foo"`）→ 無視して既知フィールドのみ反映
6. 登録 7 匹以上 → 7 匹目以降を無視して 6 匹のみ反映（警告ダイアログ等は出さない）
7. 登録 6 匹未満 → 不足分は空欄として表示
8. 反映成功 → 全 ViewModel に値を流し込み、`IsDirty = false` にリセット

**セーブ処理**:
1. 「セーブ」ボタン押下 → ViewModel 層で `Validate()` を実行
2. 不正値あり（マスターデータ非存在、能力ポイント範囲外、合計 66 超）→ エラーダイアログにフィールド別エラーを列挙し、保存しない
3. 不正値なしだが未入力・未選択あり（空欄技含む）→ 確認ダイアログ「未入力項目があります。保存を続けますか？」（はい: 続行 / いいえ・×: 中断）
4. 続行 → `PartyFileService.SaveAsync()` で `data/party.json` に書き出し
5. ファイル I/O 例外 → エラーダイアログ「保存に失敗しました: 〈例外メッセージ〉」を表示し、`IsDirty` は変更しない
6. 保存成功 → `IsDirty = false` にリセット

**ウィンドウクローズ処理**:
1. ×（ウィンドウクローズ）押下 → `IsDirty` を確認
2. `IsDirty = false` → そのまま閉じる
3. `IsDirty = true` → 確認ダイアログ「編集中の内容を保存していません。閉じますか？」（はい: 閉じる / いいえ・×: キャンセル）

### サジェスト UI 仕様

PRD「フロント（pokelens 本体）の相手ポケモン名入力と同等」を満たすため、以下を統一する:

- 入力対象: ポケモン名 / 特性 / アイテム / 技 4 欄（合計 7 入力欄/1 匹）
- 正規化: ひらがな ↔ カタカナ ↔ 半角カタカナ ↔ 半角英字（ローマ字）↔ 長音記号（半角ハイフン）。`PokelensCore.NameSearch` を呼び、JS 版 `src/logic/name-search.js` と同じ正規化規則を再実装する
- マッチング: 前方一致、最大 10 件表示
- データソース:
  - ポケモン名: `data/pokedex.json`（並び順は **図鑑番号順**。機能 3 と同じ）
  - 特性: `data/abilities.json`（並び順は **五十音順**。機能設計時点で確定）
  - アイテム: `data/items.json`（並び順は **五十音順**）
  - 技: `data/moves.json`（並び順は **五十音順**）
- UI: テキストボックスの下に Popup を表示。↑↓ キーで選択、Enter で確定、Tab は次入力欄へ移動（フロント側 SearchInput のキー操作と揃える）

### 能力ポイント補助ボタンの仕様

各能力ポイント欄（HP/Atk/Def/SpA/SpD/Spe）の右隣に 4 ボタン（「0」「−」「+」「最大」）を配置:

| ボタン | 挙動 |
|---|---|
| 0 | 入力値に関わらず `0` を設定 |
| 最大 | 入力値に関わらず `32` を設定 |
| − | 現在値が `0〜32` の整数なら `-1`。結果が範囲外（< 0）なら値変更なし。整数以外なら変更なし |
| + | 現在値が `0〜32` の整数なら `+1`。結果が範囲外（> 32）なら値変更なし。整数以外なら変更なし |

### 状態管理（ダーティフラグ）

`MainWindowViewModel.IsDirty` で「変更あり / なし」を表現する:

- `false` にリセットされるタイミング: 起動直後の初期表示完了 / ロード成功直後 / セーブ成功直後
- `true` になるタイミング: いずれかの入力欄の値が変化した時点（テキスト入力・ボタン押下・性格選択など）
- 各 `PokemonEntryViewModel` の `PropertyChanged` を `MainWindowViewModel` が購読し、`IsDirty = true` に切り替える

### 起動時の作業ディレクトリ解決

`data/party.json` のパスは「リポジトリルートからの相対パス」で参照する。`PokelensMasterDataBuilder.Common.DataPaths` と同じ方針で、起動時に `tools/PokelensPartyEditor/` 配下から相対的にリポジトリルートを探索し、`{repoRoot}/data/party.json` を絶対パス化する（共通化する場合は `PokelensCore.DataPaths` に切り出す）。

### PRD 受け入れ条件 ↔ コンポーネント責務マッピング

| PRD 受け入れ条件（要約） | 担当コンポーネント |
|---|---|
| GUI ツールを C# で実装 | `PokelensPartyEditor`（Avalonia） |
| 既存 `PokelensTools` をリネーム | `PokelensMasterDataBuilder`（旧 `PokelensTools`） |
| ロード/セーブボタン | `MainWindow.axaml` + `MainWindowViewModel.LoadCommand` / `SaveCommand` |
| 6 匹分の入力欄表示 | `PokemonEntryView.axaml × 6` + `PokemonEntryViewModel × 6` |
| 能力ポイント補助ボタン（0/−/+/最大） | `PokemonEntryViewModel.AbilityPoint{Up,Down,Reset,Max}Command` |
| サジェスト機能 | `SuggestPopup.axaml` + `PokelensCore.NameSearch` |
| 性格プルダウン | `PokemonEntryViewModel.NatureChoices`（`data/natures.json` から構築） |
| 起動時初期表示（空欄） | `MainWindowViewModel` 初期化ロジック |
| ロード時のエラー処理（不在/不正 JSON/型不正/不正要素無視/<6 匹/≥7 匹） | `PartyFileService.LoadAsync` + ViewModel 反映ロジック |
| セーブ時のバリデーション（マスター存在・範囲・合計） | `PokemonEntryViewModel.Validate` + `MainWindowViewModel.SaveCommand` |
| セーブ時の未入力確認ダイアログ | `MainWindowViewModel.SaveCommand` |
| セーブ失敗時のエラーメッセージ | `PartyFileService.SaveAsync` 例外ハンドリング |
| 「変更あり/なし」状態管理 | `MainWindowViewModel.IsDirty` |
| クローズ時の確認ダイアログ | `MainWindowViewModel.ConfirmCloseAsync` |

### テスト戦略（機能 14 部分）

- **ViewModel 層**: `PokelensPartyEditor.Tests` で xUnit によるユニットテスト。`PartyFileService` / `MasterDataService` はインターフェース化してモック注入し、入力イベント → バリデーション → ダーティフラグ遷移 → セーブ可否 などをカバー
- **PokelensCore**: `PokelensCore.Tests` で `NameSearch` の正規化規則・`MasterDataReader` のデシリアライズを単体テスト
- **View 層**: 自動テストは行わず、PRD 受け入れ条件に沿った手動確認とする（`docs/testing/e2e/manual-test-cases.md` に列挙）
- **データパイプライン**: 旧 `PokelensTools.Tests` → `PokelensMasterDataBuilder.Tests` にリネーム。テスト内容は変更なし
...
tools/
├── PokelensCore/                    # 共通クラスライブラリ（GUI 非依存）
├── PokelensCore.Tests/
├── PokelensMasterDataBuilder/       # 旧 PokelensTools。マスターデータ生成パイプライン
...
├── PokelensMasterDataBuilder.Tests/
├── PokelensPartyEditor/              # data/party.json 編集 GUI（Avalonia + MVVM）
└── PokelensPartyEditor.Tests/
```
...
テストフレームワーク: xUnit（`tools/PokelensMasterDataBuilder.Tests/` プロジェクト）
