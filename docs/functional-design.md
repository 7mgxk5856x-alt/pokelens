...
**責務**: 相手パーティ6スロットを管理し、各スロットを自分パーティと同形のカード（3列×2行グリッド、`.pokemon-card` クラスを共用）として配置する。カードの中に SearchInput を保持し、確定後はカード上にポケモン名・タイプを表示する。
...
部分計算が必要な箇所（例: `src/logic/speed-calc.js` の素早さ6パターン）では原始関数 `calcStat` を直接呼ぶ。表示のように6ステすべてが必要な箇所（`OwnPokemonDetail`）では便利関数 `calcActualStats` を使う。
...
> 相手パーティは自分パーティと同じ 3×2 のグリッドカードレイアウト。各カードに検索入力欄があり、確定後はカード上に名前・タイプが表示される。
