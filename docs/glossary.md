...
**状態モデル**: 通常 / メガシンカ の 2 状態。状態はメモリのみで管理され、リロードで初期化される。複数メガを持つポケモン（リザードン X/Y 等）の循環内容は自分側・相手側で異なる:
- 相手側: 持ち物未知のため全メガ形態を循環（例: リザードン → メガリザードンＸ → メガリザードンＹ → リザードン）
- 自分側: 持ち物に対応するメガ形態のみ循環（例: リザードン + リザードナイトＸ なら「通常 ↔ メガリザードンＸ」）

**ボタン表示条件**:
- 自分パーティ: 持ち物が当該ポケモンに対応するメガストーンの場合のみ。ただしメガストーン不要メガ (現状レックウザのみ、`megaForms[0].item === null` で表現) は持ち物に関わらず常時表示する簡略仕様
- 相手パーティ: メガシンカ可能なポケモンであれば常時（持ち物未知のため）

**関連用語**: [メガストーン](#メガストーン)、[メガフォーム](#メガフォーム)、[親ポケモン](#親ポケモン)

**実装**: `data/pokedex.json` の親エントリ内 `megaForms[]` フィールド（マスターデータに統合）、`src/data/loader.js` の `getMegaFormByName` / `getMegaFormByItem` / `isMegaForm`、`src/ui/own-party-panel.js` / `src/ui/opponent-party-panel.js`（切替 UI）

---

### メガストーン

**定義**: メガシンカの発動条件となる持ち物の総称。日本語では「〜ナイト」（例: フシギバナイト、リザードナイトＸ）として表現される。X / Y を含む名前は全角統一（`リザードナイトＸ` / `リザードナイトＹ`）。**例外**: メガレックウザは実ゲーム仕様上メガストーン不要 (ガリョウテンセイ習得が条件)。マスターデータでは `megaForms[0].item === null` で表現する。

**説明**: pokelens では火力指数に直接影響しないため `data/items.json` には登録されない。`data/pokedex.json` の親エントリ内 `megaForms[].item` フィールドが「親ポケモン + 持ち物 → 対応メガ形態」を一意に決定する。自分側パーティの切替ボタン表示判定 (`DataLoader.getMegaFormByItem(親名, アイテム名)`) で使用する。

**関連用語**: [メガシンカ](#メガシンカ)、[メガフォーム](#メガフォーム)、[親ポケモン](#親ポケモン)
...
**マスターデータ上の扱い**: `data/pokedex.json` の親エントリ内 `megaForms[]` 配列にサブデータとして物理ネストされる。各要素は `{ key, name, item, types, baseStats, abilities }` の構造を持ち、メガフォームのトップレベルエントリは存在しない。`item` は通常メガフォームで対応メガストーンの日本語名 (string)、メガストーン不要メガ (現状レックウザのみ) では `null`:
- `loader.getPokemonByName('メガフシギバナ')` は `null` を返す（`party.json` でメガ名指定は不明扱い）。トップレベルにメガキーが存在しないため自然に成立
- 相手ポケモンサジェスト候補にメガ名は含まれない（`searchByName` がトップレベルのみ走査するため）
- メガフォームの種族値・タイプ・特性は `loader.getMegaFormByName(メガ名)` または `loader.getMegaFormByItem(親名, アイテム名)` で取得する

**関連用語**: [メガシンカ](#メガシンカ)、[メガストーン](#メガストーン)、[親ポケモン](#親ポケモン)

---

### 親ポケモン

**定義**: メガシンカ可能なポケモンのうち、通常状態（メガシンカ前）のポケモンを指す。例: 「リザードン」はメガリザードンＸ / メガリザードンＹ の親ポケモン。

**マスターデータ上の表現**: `data/pokedex.json` のトップレベルエントリとして存在し、メガシンカ可能な場合は `megaForms[]` フィールドでメガフォームへの紐付けを持つ。`searchByName` でサジェストされるのは親ポケモンのみ。

**関連用語**: [メガシンカ](#メガシンカ)、[メガストーン](#メガストーン)、[メガフォーム](#メガフォーム)
