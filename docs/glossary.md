...
**こだわりスカーフ補正の扱い**: 機能 15（P0.5）で 6 パターン → 4 パターンへ簡素化。こだわりスカーフ補正後の値は「最速 × 1.5」「準速 × 1.5」で暗算容易なため一覧から除外し、自分側のスカーフ持ちポケモンについては機能 16（`OwnPokemonDetail`）で素早さ実数値に併記する。スカーフ倍率定数 `SCARF_MULTIPLIER`（=1.5）は `src/logic/speed-calc.js` の export として `OwnPokemonDetail` から参照される。
