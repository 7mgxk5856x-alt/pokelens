...
| `/update-docs <機能名> [<意図補足>]` | **PRD の変更を派生 docs に伝播するコマンド**。**PRD（`docs/product-requirements.md`）に当該機能セクションが既に書かれていることが前提**（PRD 編集は手動 or `/setup-project`）。git 状態で 2 モードを自動切替: **Phase 1（pre-impl）** = PRD の該当セクションを仕様の出典として、派生 5 ドキュメント（機能設計・アーキテクチャ・リポジトリ構造・用語集・開発ガイドライン）への反映をカテゴリ別承認で進める。**Phase 3（post-impl）** = 実装完了後の PRD 受け入れ条件チェックボックス更新・テスト一覧反映 | 更新する（承認後） | **PRD 編集 → `/add-feature` の前（Phase 1）** と **`/add-feature` の後（Phase 3）** |
| `/add-feature <機能名>` | **docs を仕様として実装する実装専念モード**。前提: PRD に機能セクションあり、`/update-docs <機能名>` で派生 docs に伝播済み。`design.md` は実装計画に縮退し、新たな設計判断はしない。docs に未記載の判断が必要になったらルールD で中断 → ユーザーが PRD 補強 → `/update-docs <機能名>` 再伝播 → 再開 | 実装する | PRD・派生 docs で設計が固まった機能ごと |
...
2. PRD 編集           docs/product-requirements.md に新機能セクションを追加
                      （ユーザーストーリー・受け入れ条件・優先度を手で書く。
                      `/setup-project` の追記モードでも可）
3. 設計記述           /update-docs <機能名>
                      └ Phase 1（pre-impl）: PRD の該当セクションを仕様の出典として、
                        派生 5 docs（functional-design・architecture・
                        repository-structure・glossary・development-guidelines）への
                        反映をカテゴリ別ユーザー承認で進める
                      └ docs が固まった時点で「設計の単一真実源」として確定
4. 実装               /add-feature <機能名>
                      └ docs を仕様として読んで実装計画（steering の tasklist）に分解
                      └ 実装ループ → test/lint → implementation-validator まで自動
                      ※ docs に書かれていない判断が出たら **ルールD で中断**して 2〜3 に戻る
5. 事後反映           /update-docs
                      └ Phase 3（post-impl）: src/・tests/ の git 差分を検出して
                        PRD 受け入れ条件チェックボックス更新・テスト一覧反映・docs 細部追従
6. コードレビュー      /review-code
                      └ 実装結果を人の目で点検し、指摘は手で修正
7. テスト仕様の点検    /review-test-cases <パス>（/update-docs が反映した一覧を点検）
8. コミット            /suggest-commit-message → git commit -F .claude/tmp/commit-msg-1.txt
9. PN 統合へマージ     P0/feature/<機能名> → P0/release
```

- **docs が設計の単一真実源** という原則を守るため、PRD 編集（2）と派生 docs への伝播（3）を実装（4）より前に置く。`/add-feature` は docs に従って実装するだけで、新たな設計判断はしない。
- `/add-feature` の **ルールD**: 実装中に docs 未記載の判断が必要になったら自己判断せず中断。ユーザーが PRD を補強（2）→ `/update-docs` 再実行（3）→ `/add-feature` 再開、の流れに戻る。
- 6〜7 は「自動実装の成果物を点検・補強する」工程に位置づく。
