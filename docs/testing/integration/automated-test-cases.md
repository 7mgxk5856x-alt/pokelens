...
| PIT-013 | 統合・異常系 | メガストーンを持たない孤立メガ（Dragon Ascent 仕様のメガレックウザ等）が最終 `pokedex.json` トップレベルから除去される（D-1 不変条件） | `showdown-pokedex.json` に `rayquaza` + `rayquazamega`（`forme:'Mega'`）＋ `showdown-moves.json`（空 `{}`）＋ `showdown-items.json`（空 `{}`、rayquazamega に対応する megaStone 無し）＋ `showdown-abilities.json`（空 `{}`）で `MergeConverter.Convert()` | `pokedex.json` に `rayquazamega` キー無し。`rayquaza` は残るが `megaForms` フィールドは JSON 上存在しない（`JsonObject` の missing key 動作。JS 側では `undefined`、MCT-030 と等価） | D-1 孤立メガ除去の Convert() 統合担保。MCT-034 は単体層 |
