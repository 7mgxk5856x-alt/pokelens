...
> 統合テストは [integration-test-cases.md](./integration-test-cases.md) に分離。手動テストの仕様書は別途 `docs/testing/manual/` に作成予定。
...
**ファイル接頭辞（フロントエンド）**: CAS=calc-actual-stats / PIC=power-index-calc / SPD=speed-calc / RMOD=resolve-modifier / NS=name-search / LD=loader

**ファイル接頭辞（C#）**: IRT=IncrementalRunnerTests / MCT=MergeConverterTests / PAT=PatchApplicatorTests / PFT=PokeAPIFetcherTests / SFT=ShowdownFetcherTests
...
| テストファイル | テスト対象 | ケース数 |
|---|---|---:|
| [calc-actual-stats.test.js](#calc-actual-statstestjs) | `src/logic/calc-actual-stats.js` | 8 |
| [power-index-calc.test.js](#power-index-calctestjs) | `src/logic/power-index-calc.js` | 8 |
| [speed-calc.test.js](#speed-calctestjs) | `src/logic/speed-calc.js` | 11 |
| [resolve-modifier.test.js](#resolve-modifiertestjs) | `src/logic/resolve-modifier.js` | 43 |
| [name-search.test.js](#name-searchtestjs) | `src/logic/name-search.js` | 27 |
| [loader.test.js](#loadertestjs) | `src/data/loader.js` | 26 |
| **小計** | | **123** |
...
| テストファイル | テスト対象 | ケース数 |
|---|---|---:|
| [IncrementalRunnerTests.cs](#incrementalrunnertestscs) | `IncrementalRunner` | 13 |
...
| [PokeAPIFetcherTests.cs](#pokeapifetchertestscs) | `PokeAPIFetcher` | 33 |
| [ShowdownFetcherTests.cs](#showdownfetchertestscs) | `ShowdownFetcher` | 15 |
| **小計** | | **92** |

**単体テスト総合計: 215 ケース**（統合テスト2件は [integration-test-cases.md](./integration-test-cases.md) を参照）
...
|---|---|---|---|---|---|
| CAS-001 | 正常系 | HP実数値の計算式 | `calcHp(108, 32)` / `calcHp(108, 0)` | `215` / `183` | base + ap + 75 |
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| CAS-002 | 正常系 | 計算式（上昇/等倍/下降） | `calcStat(130,32,1.1)` / `calcStat(102,0,1.0)` / `calcStat(80,0,0.9)` | `200` / `122` / `90` | floor((base+ap+20)×nature) |
| CAS-003 | 正常系 | 性格補正なし(1.0)で動作 | `calcStat(100,32,1.0)` | `152` | 等倍 |
...
| PIC-007 | 組合せ | 特性・持ち物補正の乗算 | `physical()`, `['Ground']`,1.3,1.2 | `46800`（=100×200×1.5×1.3×1.2、`toBeCloseTo`） | STAB+特性+持ち物 |
...
**テスト対象**: `src/logic/speed-calc.js`（素早さ6パターン計算）

### `calcSpeedPatterns(baseSpe)`

> 種族値から素早さ6パターン（最速スカーフ/準速スカーフ/最速/準速/無補正/最遅）を算出する。スカーフは「実数値の floor 後に ×1.5 して再度 floor」する。
>
> - `baseSpe`（number）: 素早さの種族値。正の整数のみ受け付け、それ以外（0・負数・NaN・小数・文字列）は `RangeError`
> - 戻り値（object: `{fastestScarf, fastScarf, fastest, fast, neutral, slowest: number}`）: 6パターンの素早さ実数値

| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| SPD-001 | 正常系 | 種族値90で6パターン算出 | `90` | `{fastestScarf:234, fastScarf:213, fastest:156, fast:142, neutral:110, slowest:99}` | 代表値 |
| SPD-002 | 境界値 | 最小付近でも破綻しない | `1` | 各式どおり（fast=53, neutral=21 等） | 種族値最小 |
| SPD-003 | 境界値 | 大きい値でも破綻しない | `200` | `neutral:220`, `slowest:floor(220×0.9)` 等 | 種族値大 |
| SPD-004 | 境界値 | スカーフ floor 順序（最速） | `85` | `fastestScarf === 225` | floor(150×1.5)。floor前×1.5なら226になる誤りを排除 |
| SPD-005 | 境界値 | スカーフ floor 順序（準速） | `91` | `fastScarf === 214` | floor(143×1.5) |
| SPD-006 | 正常系 | 最遅は0.9補正の floor | `100` | `slowest === 108` | floor((100+20)×0.9) |
| SPD-007 | 異常系 | ゼロ入力を拒否 | `0` | `RangeError` を throw | 不正入力 |
| SPD-008 | 異常系 | 負数入力を拒否 | `-1` | `RangeError` を throw | 不正入力 |
| SPD-009 | 異常系 | NaN入力を拒否 | `NaN` | `RangeError` を throw | 不正入力 |
| SPD-010 | 異常系 | 小数入力を拒否 | `90.5` | `RangeError` を throw | 非整数 |
| SPD-011 | 異常系 | 文字列入力を拒否 | `'90'` | `RangeError` を throw | 型違い |
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| LD-013 | 正常系 | カテゴリを日本語化 | `getMoveCategory('Physical')` / `getMoveCategory('Status')` | `'物理'` / `'変化'` | 英→日 |
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| LD-025 | 正常系 | 存在するポケモン名 | `getPokemonByName('ガブリアス')` | `name='ガブリアス', types=['Dragon','Ground']`（非null） | 取得成功 |
...
### `IncrementalRunner.DetermineSteps(old, current)`

> チェックサム辞書の新旧を比較し、再実行が必要なステップ（Step2/3/4）を判定する。初回（`old` が空）は全ステップ必要。
>
> - `old`（`Dictionary<string, string>`）: 前回のチェックサム（データ種別キー → ハッシュ）
> - `current`（`Dictionary<string, string>`）: 今回のチェックサム
> - 戻り値（`Steps` = `record(bool NeedsStep2, bool NeedsStep3, bool NeedsStep4)`）: 各ステップの要否

| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| IRT-001 | 正常系 | 初回は全ステップ必要 | `old={}`, `current=`全11キー | `Step2/3/4 = true` | 初回実行 |
...
| IRT-010 | 正常系 | 変更なしは全ステップ不要 | `old === current`（同一） | `Step2/3/4 = false` | 無変更 |

### `IncrementalRunner.LoadChecksums(path)`

> JSON ファイルからチェックサム辞書を読み込む。ファイルが存在しなければ空辞書。
>
> - `path`（string）: チェックサム JSON のパス
> - 戻り値（`Dictionary<string, string>`）: 読み込んだ辞書。ファイル無しは空

| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| IRT-011 | 異常系 | 存在しないファイル | ランダムな未存在パス | 空辞書 | 未存在時の安全性 |
...
> チェックサム辞書を JSON ファイルへ保存する（`LoadChecksums` と対）。
>
> - `checksums`（`Dictionary<string, string>`）: 保存する辞書
> - `path`（string）: 出力パス
> - 戻り値: なし（void）
...
| MCT-017 | 境界値 | 追加効果なしは hasSecondary なし | thunderbolt（secondary なし） | tags に `hasSecondary` を含まない | 付与されない |
...
**テスト対象**: `tools/PokelensTools/PokeAPIFetcher.cs`（PokéAPI からの和名取得・スラッグ導出）。

### `PokeAPIFetcher.DerivePokemonFormSlug(showdownName)`
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| PFT-001〜013 | 正常系 | 各種表記のスラッグ化（`Theory` 13件） | `'Bulbasaur'` / `'Rotom-Wash'` / `'Necrozma-Dusk-Mane'` / `'Tapu Koko'` / `'Mr. Mime'` / `'Mime Jr.'` / `'Type: Null'` / `'Farfetch'd'` / `'Farfetch’d-Galar'` / `'Zygarde-10%'` / `'Flabébé'` / `'Venusaur-Mega'` / `'Urshifu-Rapid-Strike'` | 各 `'bulbasaur'` / `'rotom-wash'` / `'necrozma-dusk-mane'` / `'tapu-koko'` / `'mr-mime'` / `'mime-jr'` / `'type-null'` / `'farfetchd'` / `'farfetchd-galar'` / `'zygarde-10'` / `'flabebe'` / `'venusaur-mega'` / `'urshifu-rapid-strike'` | 記号・空白・アクセント正規化 |

### `PokeAPIFetcher.DeriveItemSlug(showdownName)`
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| PFT-014〜019 | 正常系 | 各種持ち物のスラッグ化（`Theory` 6件） | `'Choice Scarf'` / `'Life Orb'` / `'Wellspring Mask'` / `'Ice Stone'` / `'Auspicious Armor'` / `'Metal Alloy'` | 各 `'choice-scarf'` / `'life-orb'` / `'wellspring-mask'` / `'ice-stone'` / `'auspicious-armor'` / `'metal-alloy'` | 空白→ハイフン・小文字化 |

### `PokeAPIFetcher.FindMatchingVariety(speciesNode, targetSlug)`
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| PFT-020 | 正常系 | 完全一致 | varieties `[rotom, rotom-wash]`, target `'rotom-wash'` | `'rotom-wash'` | 完全一致 |
| PFT-021 | 正常系 | 最長前方一致 | varieties `[ogerpon, ogerpon-wellspring-mask]`, target `'ogerpon-wellspring'` | `'ogerpon-wellspring-mask'` | 最長一致 |
| PFT-022 | 異常系 | 不一致は null | varieties `[pikachu]`, target `'raichu'` | `null` | 不一致 |
| PFT-023 | 異常系 | varieties フィールド無し | `{name:'test'}` | `null` | フィールド欠落 |

### `PokeAPIFetcher.ExtractJaName(root, arrayKey)`
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| PFT-024 | 正常系 | ja を ja-Hrkt より優先 | names に `ja` と `ja-Hrkt` 両方 | `'ピカチュウ漢字版'`（ja） | 優先順位 |
| PFT-025 | 正常系 | ja-Hrkt のみなら採用 | names に `ja-Hrkt` のみ | `'ピカチュウ'` | フォールバック |
| PFT-026 | 異常系 | 日本語なしは null | names に `en` のみ | `null` | 該当なし |
| PFT-027 | 異常系 | 配列キー欠落は null | `{name:'test'}`（names なし） | `null` | フィールド欠落 |
| PFT-028 | 正常系 | form_names キーでも動作 | `form_names` に `ja` | `'ウォッシュロトム'` | 別キー対応 |
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| PFT-029 | 正常系 | 200＋ja名 | 200 ＋ `ja` 名を含む JSON | `'ピカチュウ'` | 取得成功 |
| PFT-030 | 異常系 | 404 は null | HTTP 404 | `null` | NotFound |
| PFT-031 | 異常系 | 400 は null | HTTP 400 | `null` | BadRequest |
| PFT-032 | 異常系 | 不正 JSON は例外 | 200 ＋ `'not json'` | `JsonException` を throw | パース失敗を伝播 |
| PFT-033 | 異常系 | 日本語なしは null | 200 ＋ `en` のみ | `null` | 該当なし |
