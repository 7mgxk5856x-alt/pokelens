...
| [loader.test.js](#loadertestjs) | `src/data/loader.js` | 45 |
| **小計** | | **161** |
...
| [MergeConverterTests.cs](#mergeconvertertestscs) | `MergeConverter` | 36 |
...
**単体テスト総合計: 333 ケース**（統合テスト 14 件は [../integration/automated-test-cases.md](../integration/automated-test-cases.md) を参照）
...
| MCT-034 | 正常系 | 孤立メガ（megaStone 無し）を `forme="Mega"` で判別してトップレベルから除去 | rayquazamega をトップレベルに持ち、`showdownPokedex.rayquazamega.forme='Mega'` | `rayquazamega` がトップレベルから消える、`rayquaza` は残る（megaForms 無し） | メガレックウザ等のスペシャル仕様対応 |
