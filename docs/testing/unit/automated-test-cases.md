...
| [power-index-calc.test.js](#power-index-calctestjs) | `src/logic/power-index-calc.js` | 9 |
...
| [loader.test.js](#loadertestjs) | `src/data/loader.js` | 28 |
| **小計** | | **129** |
...
**単体テスト総合計: 223 ケース**（統合テスト2件は [../integration/automated-test-cases.md](../integration/automated-test-cases.md) を参照）
...
|---|---|---|---|---|---|
| CAS-001 | 正常系 | HP実数値の計算式（能力ポイント最大値 ap=32） | `calcHp(108, 32)` | `215` | base + ap + 75 |
| CAS-001b | 境界値 | HP実数値の計算式（能力ポイント最小値 ap=0） | `calcHp(108, 0)` | `183` | base + 0 + 75 |
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| CAS-002 | 正常系 | 上昇補正（nature=1.1） | `calcStat(130,32,1.1)` | `200` | floor((130+32+20)×1.1) |
| CAS-002b | 正常系 | 等倍補正（nature=1.0） | `calcStat(102,0,1.0)` | `122` | (102+0+20)×1.0 |
| CAS-002c | 正常系 | 下降補正（nature=0.9） | `calcStat(80,0,0.9)` | `90` | floor((80+0+20)×0.9) |
...
| SPD-002 | 境界値 | 種族値最小値で全 6 パターン算出 | `1` | `{fastestScarf:87, fastScarf:79, fastest:58, fast:53, neutral:21, slowest:18}` | floor の連鎖が破綻しない |
