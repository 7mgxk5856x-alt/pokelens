...
| [loader.test.js](#loadertestjs) | `src/data/loader.js` | 44 |
| **小計** | | **160** |
...
| [MergeConverterTests.cs](#mergeconvertertestscs) | `MergeConverter` | 35 |
...
| [ShowdownFetcherTests.cs](#showdownfetchertestscs) | `ShowdownFetcher` | 17 |
| [ShowdownInternalKeyTests.cs](#showdowninternalkeytestscs) | `ShowdownInternalKey` | 59 |
| **小計** | | **167** |

**単体テスト総合計: 327 ケース**（統合テスト 8 件は [../integration/automated-test-cases.md](../integration/automated-test-cases.md) を参照）
...
| LD-027c | 異常系 | メガシンカ不可のポケモンは `megaForms` フィールド未定義 | `getPokemonByName('メタモン').megaForms` | `undefined` | 非対象 |
...
| MCT-027 | 正常系 | メガ名の半角 X/Y → 全角 Ｘ/Ｙ 正規化 | charizardmegay 入力 `name='メガリザードンY'`（半角 Y） | 出力 `name='メガリザードンＹ'`（全角 Ｙ）、半角 Y を含まない | D-8 |
...
| MCT-030 | 異常系 | メガシンカ不可ポケモンは `megaForms` を持たない | raichu（megaStone 該当なし） | `raichu.megaForms` が `null` | 非対象 |
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| SFT-012 | 異常系 | 非標準持ち物（megaStone 無し）は除外 | `isNonstandard:'Past'` | `null` | 除外 |
...
| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| SIK-001〜049 | 正常系 | Gen 6/7 公式メガ全 49 形態の変換を網羅（`Theory` 49 件） | `"Venusaur-Mega"`, `"Charizard-Mega-X"`, `"Charizard-Mega-Y"`, ..., `"Diancie-Mega"` | 各 `"venusaurmega"`, `"charizardmegax"`, `"charizardmegay"`, ..., `"dianciemega"` | D-7。命名規則外のメガと items.ts ↔ pokedex.json 間のキー drift を即時検知 |
| SIK-050〜058 | 正常系 | メガ以外の特殊表記の汎用性確認（`Theory` 9 件） | `"Bulbasaur"` / `"Rotom-Wash"` / `"Necrozma-Dusk-Mane"` / `"Tapu Koko"` / `"Type: Null"` / `"Mr. Mime"` / `"Farfetch'd"` / `"Flabébé"` / `"Urshifu-Rapid-Strike"` | 各 `"bulbasaur"` / `"rotomwash"` / `"necrozmaduskmane"` / `"tapukoko"` / `"typenull"` / `"mrmime"` / `"farfetchd"` / `"flabebe"` / `"urshifurapidstrike"` | 変換ロジックの汎用性確認（ハイフン除去・スペース除去・約物除去・アクセント正規化） |
