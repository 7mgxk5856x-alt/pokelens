...
- **対象**: `tools/PokelensMasterDataBuilder.Tests/*.cs`（`PipelineIntegrationTests.cs` を除く）
...
**ファイル接頭辞（C#）**: IRT=IncrementalRunnerTests / MCT=MergeConverterTests / PAT=PatchApplicatorTests / PFT=PokeAPIFetcherTests / PNM=PokeApiNameTests / PSL=PokeApiSlugTests / SFT=ShowdownFetcherTests / SIK=ShowdownInternalKeyTests
...
| [loader.test.js](#loadertestjs) | `src/data/loader.js` | 49 |
| **小計** | | **165** |
...
| [MergeConverterTests.cs](#mergeconvertertestscs) | `MergeConverter` | 37 |
...
| [ShowdownFetcherTests.cs](#showdownfetchertestscs) | `ShowdownFetcher` | 20 |
| [ShowdownInternalKeyTests.cs](#showdowninternalkeytestscs) | `ShowdownInternalKey` | 60 |
| **小計** | | **173** |

**単体テスト総合計: 338 ケース**（統合テスト 14 件は [../integration/automated-test-cases.md](../integration/automated-test-cases.md) を参照）
...
| LD-027 | 正常系 | メガフォーム名で `null` 返却（機能 7: party.json でメガ名指定は不明扱い） | `getPokemonByName('メガフシギバナ')` / `getPokemonByName('メガリザードンＸ')` / `getPokemonByName('メガリザードンＹ')` | いずれも `null` | PRD 機能 7。メガフォームは親エントリの `megaForms[]` に物理ネストされ pokedex.json トップレベルに存在しないため自然に null 返却（P0.5 リファクタ） |
| LD-027b | 正常系 | 親ポケモンは `megaForms[]` フィールドを持つ（P0.5 リファクタ） | `getPokemonByName('フシギバナ').megaForms` | 配列長 1、`[0].key='venusaurmega'`, `[0].name='メガフシギバナ'`, `[0].item='フシギバナイト'` | 新スキーマの親エントリにネスト済み |
| LD-027c | 正常系 | メガシンカ不可のポケモンは `megaForms` フィールド未定義（オプショナルフィールドが省略される正常動作） | `getPokemonByName('メタモン').megaForms` | `undefined` (機能設計書の型定義 `megaForms?` どおりフィールド省略。C# 層では `null` 返却 = MCT-030 と等価) | 非対象。MCT-030 と同じ事実を JS 層で表現 |

### `DataLoader.isMegaForm(name)` / `getMegaFormByName(megaName)` / `getMegaFormByItem(parentName, itemName)`（機能 7、P0.5 リファクタ）

> メガシンカ関連 API。`data/pokedex.json` の親エントリ `megaForms[]` を真実源とし、loader 初期化時に派生 Map `#megaLocationByName` を構築して O(1) ルックアップを実現する。
>
> - `isMegaForm(name)`: 名前がメガフォームなら `true`
> - `getMegaFormByName(megaName)`: メガ名から親にネストされた MegaFormData (`{key, name, item, types, baseStats, abilities}`) を引く。通常名・未登録名なら `null`
> - `getMegaFormByItem(parentName, itemName)`: 親名 + 持ち物名から対応メガを引く（PRD 260-264 の自分側トグル表示判定に使用）

| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| LD-028 | 正常系 | isMegaForm: メガフォーム名 | `isMegaForm('メガフシギバナ')` / `isMegaForm('メガリザードンＸ')` / `isMegaForm('メガリザードンＹ')` | `true` | 判定 |
| LD-029 | 異常系 | isMegaForm: 通常名・未登録名 | `isMegaForm('フシギバナ')` / `isMegaForm('メタモン')` / `isMegaForm('存在しないポケモン')` | `false` | 非対象 |
| LD-030 | 正常系 | getMegaFormByName: メガ名で MegaFormData を返す | `getMegaFormByName('メガフシギバナ')` | 非null かつ `key='venusaurmega'`, `name='メガフシギバナ'`, `item='フシギバナイト'`, `baseStats.atk===100`, `abilities=['あついしぼう']` | 取得成功 |
| LD-030b | 正常系 | getMegaFormByName: item: null メガ（メガレックウザ）でも MegaFormData を返す（D-1 nullable 担保） | `getMegaFormByName('メガレックウザ')` | 非null、`key='rayquazamega'`, `name='メガレックウザ'`, `item===null`, `baseStats.atk===180`, `abilities=['デルタストリーム']` | D-1 nullable 担保。`item` フィールドが null の MegaFormData も他のメガと同様に取得できる |
| LD-031 | 正常系 | getMegaFormByName: 複数メガから個別形態を引ける | `getMegaFormByName('メガリザードンＸ')` / `getMegaFormByName('メガリザードンＹ')` | X は `types=['Fire','Dragon']`, `item='リザードナイトＸ'`。Y は `types=['Fire','Flying']`, `item='リザードナイトＹ'` | 複数フォーム識別 |
| LD-032 | 異常系 | getMegaFormByName: 通常名・未登録名 | `getMegaFormByName('フシギバナ')` / `getMegaFormByName('メタモン')` / `getMegaFormByName('存在しないポケモン')` | `null` | 非対象 |
| LD-033 | 正常系 | getMegaFormByItem: 親名 + 対応メガストーンで MegaFormData を返す | `getMegaFormByItem('フシギバナ', 'フシギバナイト')` | 非null、`name='メガフシギバナ'` | 標準パターン |
| LD-033b | 正常系 | getMegaFormByItem: 複数メガで持ち物に応じた形態 | `getMegaFormByItem('リザードン', 'リザードナイトＸ')` / `getMegaFormByItem('リザードン', 'リザードナイトＹ')` | `name='メガリザードンＸ'` / `name='メガリザードンＹ'` | 持ち物別の解決 |
| LD-034 | 異常系 | getMegaFormByItem: 対応外メガストーン | `getMegaFormByItem('フシギバナ', 'カメックスナイト')` | `null` | PRD 260-264 の不一致ケース |
| LD-034b | 異常系 | getMegaFormByItem: メガストーンでない持ち物 | `getMegaFormByItem('フシギバナ', 'こだわりハチマキ')` | `null` | PRD 260-264 の汎用持ち物ケース |
| LD-034c | 異常系 | getMegaFormByItem: メガ不可ポケモン | `getMegaFormByItem('メタモン', 'フシギバナイト')` | `null` | 非対象 |
| LD-034d | 境界値 | getMegaFormByItem: 持ち物が null | `getMegaFormByItem('リザードン', null)` / `getMegaFormByItem('フシギバナ', null)` | いずれも `null` | party.json で持ち物未指定（item: null）のポケモンでメガトグルが非表示になる仕様（PRD 260-264）を担保 |
| LD-034e | 異常系 | getMegaFormByItem: 存在しないポケモン名 | `getMegaFormByItem('存在しないポケモン', 'フシギバナイト')` | `null`（TypeError を throw しない） | UI フォールバック層（OwnPartyPanel メガシンカ切替ボタン）の前提条件を保証 |
| LD-034f | 正常系 | getMegaFormByItem: メガストーン不要メガ（item: null）は持ち物に関わらずメガフォームを返す（D-5 フォールバック） | `getMegaFormByItem('レックウザ', 'いのちのたま')` / `getMegaFormByItem('レックウザ', null)` / `getMegaFormByItem('レックウザ', '')` / `getMegaFormByItem('レックウザ', 'フシギバナイト')` | いずれも `name='メガレックウザ'` を返す | D-5 / PRD 機能 7「メガストーン不要メガの簡略仕様」。技習得チェックなし |
| LD-034g | 正常系 | getMegaFormByItem: 持ち物一致メガが存在するときは item: null フォールバックより優先 | `getMegaFormByItem('フシギバナ', 'フシギバナイト')` | 非null、`item='フシギバナイト'`（フシギバナには item:null メガが無いため項 1 のみ発動） | D-5 優先順位の単一真実源 |
| LD-034h | 境界値 | getMegaFormByItem: レックウザの megaForms[0].item は null である（fixture 整合性チェック） | `getPokemonByName('レックウザ').megaForms` | 配列長 1、`[0].item === null`, `[0].name='メガレックウザ'` | D-1 スキーマ拡張担保。SAMPLE_POKEDEX が item: null を含むことを保証 |
| LD-035 | 正常系 | searchByName: メガフォームをサジェスト除外（トップレベル走査による自然な除外） | `searchByName('メガ')` | `[]` | 機能 7 P0.5 リファクタ後はメガが pokedex.json トップレベルに存在しないため自然に除外される（フィルタ不要） |
| LD-036 | 正常系 | searchByName: 親ポケモン名は通常通り候補に含まれる | `searchByName('リザード')` | `[{name:'リザードン',...}]` のみ | メガが除外されても親は含まれる |

---

## IncrementalRunnerTests.cs

**テスト対象**: `tools/PokelensMasterDataBuilder/IncrementalRunner.cs`（増分実行判定）。チェックサムの新旧比較で再実行すべきステップを決める。
...
**テスト対象**: `tools/PokelensMasterDataBuilder/MergeConverter.cs`（Showdown 生データ → アプリ用 JSON への変換）。
...
### `MergeConverter.NestMegaForms(flatPokedex, showdownPokedex, showdownItems, itemNames, itemNamePatch)`（機能 7、P0.5 リファクタ）

> 平坦化されたメガ独立エントリを親エントリの `megaForms[]` にネストし、トップレベルから削除する。`items.ts` の `megaStone` フィールドを真実源として (親, メガ, メガストーン) の 3 つ組を抽出。メガ名・アイテム名の半角 X/Y は全角に正規化する（D-8）。対応する `megaStone` を持たない孤立メガ（例: メガレックウザは Dragon Ascent 仕様）は Showdown 側の `forme="Mega*"` で判別してトップレベルから除去する。
>
> - `flatPokedex`（JsonObject）: `ConvertPokedex` の戻り値（メガフォームはまだトップレベル）
> - `showdownPokedex`（JsonObject）: 孤立メガ判別用の Showdown 由来データ（`forme` フィールド参照）
> - `showdownItems`（JsonObject）: `megaStone` フィールドを持つアイテムを含む cache 出力
> - `itemNames`（JsonObject）: アイテム翻訳辞書
> - `itemNamePatch`（JsonObject）: アイテム日本語名の上書きパッチ
> - 戻り値（JsonObject）: メガがネストされ、メガ独立エントリ・孤立メガがトップレベルから削除されたポケデックス

| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| MCT-024 | 正常系 | メガを親の `megaForms[]` にネストし、key と item を含める | venusaurite (megaStone) + venusaurmega エントリ | `venusaur.megaForms[0]` が `{key:'venusaurmega', item:'フシギバナイト', name:'メガフシギバナ'}` を含む | D-1 / D-3 |
| MCT-025 | 正常系 | メガをトップレベルから削除 | 同上 | `venusaurmega` / `charizardmegax` / `charizardmegay` がトップレベルに存在しない | D-1 |
| MCT-026 | 正常系 | 複数メガ（リザードン）を同じ親にネスト | charizarditex + charizarditey | `charizard.megaForms` が 2 件、`key='charizardmegax'` と `key='charizardmegay'` を両方含む | 複数フォーム |
| MCT-027 | 正常系 | メガ名の半角 X/Y → 全角 Ｘ/Ｙ 正規化（X / Y 両方対称的に検証） | charizardmegax 入力 `name='メガリザードンX'`（半角 X）, charizardmegay 入力 `name='メガリザードンY'`（半角 Y） | 出力 X 側 `name='メガリザードンＸ'`・Y 側 `name='メガリザードンＹ'`（全角）、いずれも半角 X/Y を含まない | D-8（X / Y の対称性も担保） |
| MCT-028 | 正常系 | メガストーン名の半角 X/Y → 全角 Ｘ/Ｙ 正規化 | itemNames `{charizarditex:'リザードナイトX', charizarditey:'リザードナイトY'}`（半角） | 出力 `item='リザードナイトＸ'` / `'リザードナイトＹ'`（全角） | D-8 |
| MCT-029 | 正常系 | メガの types/baseStats/abilities を保持 | venusaurite + venusaurmega エントリ | `megaForms[0]` の `baseStats.atk===100`, `abilities=['あついしぼう']`, `types=['Grass','Poison']` | スキーマ整合 |
| MCT-030 | 正常系 | メガシンカ不可ポケモンは `megaForms` フィールドを持たない（オプショナルフィールドが省略される正常動作） | raichu（megaStone 該当なし） | `result["raichu"]!["megaForms"]` が `null` (`JsonObject` の missing key 動作。最終 JSON 出力ではフィールド省略 → JS では `undefined` に対応、機能設計書の型定義 `megaForms?` と整合) | 非対象。LD-027c と同じ事実を C# 層で表現 |
| MCT-031 | 異常系 | メガ独立エントリが pokedex 側に無い場合はスキップ | gyaradosite (megaStone) のみ pokedex に gyarados/gyaradosmega なし | 親 `gyarados` 追加なし、メガ独立エントリも無し | データ不整合の防御 |
| MCT-032 | 正常系 | item-name-patch.json が翻訳を上書き | venusaurite に patch `'パッチ済みフシギバナイト'` | `venusaur.megaForms[0].item` がパッチ値 | パッチ優先 |
| MCT-033 | 境界値 | 純粋関数性: 入力 `flatPokedex` を変更しない | 任意の有効入力 | 元 `flatPokedex` に `venusaurmega` が残り、`venusaur.megaForms` は `null` | ディープクローンで作業 |
| MCT-034 | 正常系 | `ItemlessMegas` に登録されたメガ（メガレックウザ）は `item: null` で親エントリの `megaForms[]` にネストされる（D-4） | rayquaza + rayquazamega をトップレベルに持ち、`showdownPokedex.rayquazamega.forme='Mega'`、`showdownItems` に rayquaza 対応の megaStone 無し | `rayquazamega` がトップレベルから消え、`rayquaza.megaForms[0]` が `{key:'rayquazamega', name:'メガレックウザ', item:null（フィールド存在＋JSON null）, baseStats.atk=180, abilities=['デルタストリーム']}` | D-4 簡略仕様。技習得チェックは行わない。Q-7 ルール例外として `ItemlessMegas` Dictionary ハードコード |
| MCT-035 | 境界値 | `forme="Mega"` で始まらない名前（ガラル等）は除去対象外 | mrmimegalar、`forme='Galar'` | `mrmimegalar` がトップレベルに残る | 誤削除防止 |
| MCT-036 | 境界値 | `forme="Mega-X"` / `"Mega-Y"` のハイフン付き値も孤立メガとして除去される | charizardmegax、`forme='Mega-X'`、megaStone 無し | `charizardmegax` がトップレベルから除去される | D-1 不変条件: `StartsWith("Mega")` でメガ識別。完全一致のみではない。現行 Showdown データで "Mega" 始まりの非メガ forme は存在しないため誤削除リスク無し（将来追加された場合は MCT-035 と同様に判別条件を見直す） |
| MCT-037 | 組合せ | `ItemlessMegas` のメガは親が既に megaStone 経由で `megaForms[]` を持っていても末尾追加される（D-4 共存挙動） | 親 rayquaza が megaStone 経由のメガを 1 件保持した状態で `ItemlessMegas` ループが動く（fixture で擬似的に既存 megaForms[] を作成） | `rayquaza.megaForms` が長さ 2（既存 1 件 + ItemlessMegas 経由 1 件）、追加メガの `item===null` | 将来 `ItemlessMegas` の親に別経路 megaStone が追加された場合の回帰防止 |

---

## PatchApplicatorTests.cs

**テスト対象**: `tools/PokelensMasterDataBuilder/PatchApplicator.cs`（マスターデータ JSON へのパッチ部分適用）。
...
**テスト対象**: `tools/PokelensMasterDataBuilder/Fetchers/PokeAPIFetcher.cs`（PokéAPI からの和名取得。HTTP オーケストレーション）。
...
**テスト対象**: `tools/PokelensMasterDataBuilder/Common/PokeApiName.cs`（PokéAPI レスポンス JSON からの和名・一致フォルム抽出。純粋ロジック）。
...
**テスト対象**: `tools/PokelensMasterDataBuilder/Common/PokeApiSlug.cs`（Showdown 名 → PokéAPI slug 変換。純粋ロジック）。
...
**テスト対象**: `tools/PokelensMasterDataBuilder/ShowdownFetcher.cs`（Showdown の JS データ取得・JSON 化）。
...
> Showdown 持ち物エントリを変換する。`isNonstandard` の持ち物は null で除外。ただし `megaStone` フィールドを持つアイテム（メガストーン）は機能 7 のため `isNonstandard:"Past"` でも例外として通す（D-6）。
>
> - `entry`（JsonObject）: Showdown 持ち物エントリ
> - 戻り値（`JsonObject?`）: 変換後エントリ。非標準（megaStone 無し）は null

| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| SFT-012 | 異常系 | 非標準持ち物（megaStone フィールド無し）は除外 | `{isNonstandard:'Past', name:'Past Berry'}`（megaStone フィールド無し） | `null` | D-6 デシジョン: SFT-012〜019 を「`isNonstandard` × `megaStone` の有無」マトリクスとして読む |
| SFT-013 | 正常系 | 通常持ち物は変換 | 通常の持ち物エントリ（`isNonstandard` 無し・`megaStone` 無し） | 非null、`name='Choice Scarf'` | 取得成功 |
| SFT-016 | 正常系 | megaStone 持ちは Past でも例外通過、megaStone を保持（D-6） | absolite `{megaStone:{Absol:'Absol-Mega'}, isNonstandard:'Past'}` | 非null、`name='Absolite'`、`megaStone.Absol='Absol-Mega'` を保持 | 機能 7 リファクタ |
| SFT-017 | 正常系 | megaStone マップが複数親に対応 | charizardite x `{megaStone:{Charizard:'Charizard-Mega-X'}, isNonstandard:'Past'}` | `megaStone.Charizard='Charizard-Mega-X'` | 同上 |
| SFT-013b | 正常系 | `isNonstandard` フィールド無し + megaStone 有り（デシジョンテーブル欠落セル補完） | `{megaStone:{Foo:'Foo-Mega'}, name:'Future Mega Stone'}`（isNonstandard 無し） | 非null、`megaStone.Foo='Foo-Mega'` を保持 | 「megaStone 有り → 必ず通す」の不変条件が `isNonstandard` の有無によらず維持されることを担保 |
| SFT-018 | 異常系 | Past 以外の `isNonstandard` 値（Unobtainable 等）も megaStone 無しなら除外（D-6 同値分割の補完） | `{isNonstandard:'Unobtainable', name:'Cherish Ball'}`（megaStone 無し） | `null` | SFT-012 の同値クラス補完（Past だけが除外対象ではないことを担保） |
| SFT-019 | 境界値 | megaStone 持ちは Past 以外の `isNonstandard` 値（Future 等）でも例外通過（D-6 境界値） | `{megaStone:{Pikachu:'Pikachu-Mega'}, isNonstandard:'Future'}` | 非null、`megaStone.Pikachu='Pikachu-Mega'` を保持 | SFT-016/017 の境界値補完（「megaStone 有り → 通す」の不変条件を維持） |
...
## ShowdownInternalKeyTests.cs

**テスト対象**: `tools/PokelensMasterDataBuilder/Fetchers/ShowdownInternalKey.cs`（Showdown 表示名 → pokedex.json 内部キー変換、機能 7 P0.5 リファクタで新規追加）。

### `ShowdownInternalKey.ForPokemon(showdownName)`

> Showdown のポケモン表示名（例: `"Venusaur-Mega"`, `"Charizard-Mega-X"`）を pokedex.json のトップレベル内部キー（例: `"venusaurmega"`, `"charizardmegax"`）に変換する。`PokeApiSlug.PokemonFormSlug` の結果からハイフンを除去した形と等しい。`items.ts` の `megaStone` フィールドから pokedex.json キーへ変換する目的で導入（D-7）。
>
> - `showdownName`（string）: Showdown 表示名
> - 戻り値（string）: pokedex.json の内部キー

| ケースID | 種別 | テスト観点 | 入力値 | 期待結果 | 備考 |
|---|---|---|---|---|---|
| SIK-001〜050 | 正常系 | Gen 6/7 公式メガ全 50 形態の変換を網羅（`Theory` 50 件） | `"Venusaur-Mega"`, `"Charizard-Mega-X"`, `"Charizard-Mega-Y"`, `"Raichu-Mega-X"`, `"Raichu-Mega-Y"`, ..., `"Diancie-Mega"` | 各 `"venusaurmega"`, `"charizardmegax"`, `"charizardmegay"`, `"raichumegax"`, `"raichumegay"`, ..., `"dianciemega"` | D-7。命名規則外のメガと items.ts ↔ pokedex.json 間のキー drift を即時検知。Raichu X/Y は Showdown 独自の非公式メガだが items.ts に megaStone を持つため網羅対象 |
| SIK-051〜059 | 正常系 | メガ以外の特殊表記の汎用性確認（`Theory` 9 件） | `"Bulbasaur"` / `"Rotom-Wash"` / `"Necrozma-Dusk-Mane"` / `"Tapu Koko"` / `"Type: Null"` / `"Mr. Mime"` / `"Farfetch'd"` / `"Flabébé"` / `"Urshifu-Rapid-Strike"` | 各 `"bulbasaur"` / `"rotomwash"` / `"necrozmaduskmane"` / `"tapukoko"` / `"typenull"` / `"mrmime"` / `"farfetchd"` / `"flabebe"` / `"urshifurapidstrike"` | 変換ロジックの汎用性確認（ハイフン除去・スペース除去・約物除去・アクセント正規化） |
| SIK-060 | 境界値 | 空文字を安全に処理 | `ShowdownInternalKey.ForPokemon("")` | `""`（TypeError を throw しない） | 防御: items.ts の megaStone マップで空文字が紛れた場合等 |
