...
| テストファイル | テスト対象 | ケース数 |
|---|---|---:|
| [own-party-display.spec.js](#own-party-displayspecjs) | 自分パーティの一覧表示・選択・状態遷移・メガシンカ切替 | 11 |
...
| [opponent-pokemon-detail.spec.js](#opponent-pokemon-detailspecjs) | 相手ポケモン詳細・素早さ 4 パターン・耐久指数 4 パターン・切替・メガシンカ切替 | 6 |
...
| AET-043 | 正常系 | 機能7 | メガストーン不要メガ（メガレックウザ）は持ち物に関わらず自分側でも切替ボタンが表示される（D-4/D-5） | RAYQUAZA_FIXTURE（レックウザ + いのちのたま / レックウザ + 持ち物なし） | カード 0/1 ともに `.mega-toggle` 表示。レックウザ切替で `.name` が「レックウザ」↔「メガレックウザ」 | PRD 機能 7「メガストーン不要メガの簡略仕様」。技習得チェックは行わない |
| AET-044 | 正常系 | 機能7 | メガレックウザ切替時に自分ポケモン詳細パネルが連動更新される（item: null メガの詳細表示） | RAYQUAZA_FIXTURE のレックウザを選択後、メガ切替を押下 | 詳細の `.detail-header .name` が「レックウザ」→「メガレックウザ」、特性が「エアロック」→「デルタストリーム」へ更新 | PRD 機能 7。item: null のメガでも詳細パネル連動が機能することを担保 |
...
| AET-045 | 正常系 | 機能7 | 相手側メガレックウザはメガストーン不要メガとして循環対象に含まれる（item: null メガ） | レックウザ確定 → メガ切替 2 回（通常→メガ→通常） | スロットに `.mega-toggle` 表示、押下で info の `.name` と詳細の `.detail-header .name` が連動更新（レックウザ↔メガレックウザ） | PRD 機能 7。相手側は元々全形態を循環するため、item: null メガも AET-041 と同等の挙動になることを担保 |
