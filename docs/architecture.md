...
| 技術 | バージョン | 用途 | 選定理由 |
|------|-----------|------|----------|
| .NET | 8.0 (LTS) | ランタイム（全 C# プロジェクト共通） | 2026年11月まで長期サポート。HttpClient・System.Text.Json が標準搭載でデータ取得・変換に必要な機能が揃っている |
| HttpClient | 標準 | Showdownデータ取得 | .NET 標準。外部ライブラリ不要 |
| System.Text.Json | 標準 | JSON変換（読み書き双方） | .NET 標準。高速で依存なし。`PokelensCore.MasterDataReader` と `PartyFileService` の両方で利用 |
| Avalonia UI | 11.x | GUI フレームワーク（機能 14 / `PokelensPartyEditor`） | クロスプラットフォーム（Linux/WSL/Windows/Mac）。dev container（WSL2 Linux）で開発・ビルド可能。XAML + MVVM を標準サポート |
| CommunityToolkit.Mvvm | 8.x | MVVM 基盤（機能 14） | `ObservableObject` / `RelayCommand` を提供しボイラープレートを削減。Microsoft 公式メンテナンス |
...
### C# GUI ツール: パーティ編集（機能 14）

データ準備パイプラインとは独立した Avalonia デスクトップアプリ `PokelensPartyEditor` が `data/party.json` を編集する。フロントエンド（pokelens 本体）とは `data/party.json` ファイル経由のみで連携し、GUI 保存後にユーザーが手動でブラウザをリロードする（GUI からの自動通知は行わない）。

**プロジェクト分離**:

| プロジェクト | 種別 | 役割 |
|---|---|---|
| `tools/PokelensCore` | クラスライブラリ | サジェスト・マスターデータ読み込み・共通型。GUI フレームワーク非依存 |
| `tools/PokelensMasterDataBuilder` | コンソールアプリ | 旧 `PokelensTools`。`cache/` 経由でマスターデータを生成 |
| `tools/PokelensPartyEditor` | Avalonia デスクトップアプリ | `data/party.json` 編集 GUI。MVVM 構造 |

**依存方向**: `PokelensMasterDataBuilder` → `PokelensCore` ← `PokelensPartyEditor`。`PokelensCore` は他に依存せず、Avalonia 等 GUI 依存も持ち込まない（GUI 非依存原則）。

**MVVM 内部構成**: 詳細は `docs/functional-design.md` の「C# パーティ編集 GUI ツール（機能 14）」を参照。
...
**パーティ編集 GUI のデータフロー（機能 14）**:

```mermaid
sequenceDiagram
    participant User as ユーザー
    participant PE as PokelensPartyEditor (Avalonia)
    participant Core as PokelensCore
    participant FS as data/

    Note over PE,FS: 起動時は自動ロードせず空欄表示
    PE-->>User: 初期画面（6 匹空欄）

    Note over User,FS: ロード（任意契機）
    User->>PE: 「ロード」ボタン押下
    PE->>Core: MasterDataReader.LoadAsync()（サジェスト用）
    Core->>FS: pokedex / abilities / items / moves / natures を読み込み
    PE->>FS: data/party.json を読み込み
    PE-->>User: 画面に反映（IsDirty=false）

    Note over User,FS: セーブ
    User->>PE: 「セーブ」ボタン押下
    PE->>PE: バリデーション（マスター存在・能力ポイント範囲）
    alt 不正値あり
        PE-->>User: エラーダイアログ（保存しない）
    else 未入力あり
        PE-->>User: 確認ダイアログ
        User->>PE: 「はい」/「いいえ」
    else
        PE->>FS: data/party.json に書き出し
        PE-->>User: IsDirty=false にリセット
    end

    Note over User,FS: フロントエンドへの反映はユーザー操作（ブラウザリロード）
```
...
│   ├── PokelensCore.csproj
│   ├── NameSearch.cs                # JS 版 src/logic/name-search.js の C# 移植（正規化・前方一致）
│   ├── MasterDataReader.cs          # data/pokedex.json 等を強い型でデシリアライズ
│   └── Models/                      # PartyDocument / PokemonEntryModel / AbilityPointsModel 等
...
├── PokelensMasterDataBuilder.Tests/  # xUnit（旧 PokelensTools.Tests のリネーム）
├── PokelensPartyEditor/             # Avalonia デスクトップアプリ（機能 14・MVVM）
│   ├── PokelensPartyEditor.csproj
│   ├── App.axaml / Program.cs       # Avalonia アプリ起動エントリ
...
└── PokelensPartyEditor.Tests/       # xUnit（ViewModel 層のユニットテスト中心）
...
| 種別 | 配置先 | 生成・管理方法 |
|---|---|---|
| マスターデータ | `data/` 配下 | `PokelensMasterDataBuilder` で Showdown / PokéAPI から自動生成・再生成可能 |
| 手動補正・パッチデータ | `tools/PokelensMasterDataBuilder/Patches/` | `PokelensMasterDataBuilder` がマスターデータ生成時に適用 |
| 自分パーティ | `data/party.json` | ユーザーが GUI（`PokelensPartyEditor`）または手書きで編集。フロントエンドはこのファイルのみを参照する |
| フロントエンドのコード | `src/` 配下 | データ参照は `data/` 配下のファイルのみ。`src/data/` にもコード (`loader.js` 等) のみを置き、JSON 等のデータファイルは置かない |
| C# コード | `tools/` 配下 | 3 プロジェクト（`PokelensCore` / `PokelensMasterDataBuilder` / `PokelensPartyEditor`）。GUI 依存は `PokelensPartyEditor` のみが持つ |
...
- **補正データもパイプライン経由**: フロントエンドが特定の補正データに依存する場合、その補正は必ず `tools/PokelensMasterDataBuilder/Patches/` を経由してマスターデータに統合する。これにより全体整合性が保たれる
...
| ユニットテスト（C#・パイプライン） | xUnit | `PokelensMasterDataBuilder` の MergeConverter・パッチ適用・増分実行判定・日本語キー変換 |
| ユニットテスト（C#・共通ライブラリ） | xUnit | `PokelensCore` の `NameSearch` 正規化・`MasterDataReader` デシリアライズ |
| ユニットテスト（C#・GUI） | xUnit | `PokelensPartyEditor` の **ViewModel 層**（バリデーション・ダーティフラグ・コマンド可否）。View（XAML）は自動テスト対象外 |
| 統合テスト（C#） | xUnit | パイプライン全体のスナップショットテスト |
| E2E テスト（Web） | Playwright（Chromium） | UI 結合・画面表示・サジェスト・火力指数 UI 反映・XSS 耐性 |
| 手動テスト | ブラウザ / GUI | 環境セットアップ・C# データ生成・dev サーバー起動・パーティ編集 GUI の View 層（[`docs/testing/e2e/manual-test-cases.md`](./testing/e2e/manual-test-cases.md)） |
...
テストファイルの配置: JS テストは `tests/unit/`（ユニット）および `tests/e2e/`（E2E、Playwright）、C# テストは `tools/PokelensCore.Tests/` / `tools/PokelensMasterDataBuilder.Tests/` / `tools/PokelensPartyEditor.Tests/` の 3 プロジェクト（詳細は `docs/repository-structure.md` を参照）
