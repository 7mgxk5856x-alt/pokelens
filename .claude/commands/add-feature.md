...
2. **`main`（デフォルトブランチ）上だった場合のみ**、最終的な完了報告の末尾に次の注意を含める:
  > ⚠️ main 上で実装しました。`/add-feature` はコミットしないため、コミット前に `git switch -c feature/<機能名>` を推奨します（未コミットの変更は新ブランチへ移動し、main はクリーンのまま手戻りなく分離できます）。
3. すでに `feature/*` や `chore/*` などのブランチ上であれば、何も付記しない。
