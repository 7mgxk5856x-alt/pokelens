...
1. **自動テストケース一覧への反映（テストを追加・変更した場合は必須）**: 今回追加・変更した自動テストを、対応するスナップショット一覧へ `Edit` ツールで反映する。
  ...
  - 各テストについて、その一覧の表形式（ケースID `<ファイル接頭辞>-<連番>` / 種別 / テスト観点 / 入力値 / 期待結果 / 備考）に行を追加・更新する。
  - サマリ表の「ケース数」「総合計」も更新する。新規テストファイルには新しいファイル接頭辞を割り当て、接頭辞一覧に追記する。
