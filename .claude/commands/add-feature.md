...
2. **手動・E2E 仕様書の確認**: 今回の機能が PRD の受け入れ条件に新しいユーザー操作・画面挙動を加える場合のみ、`docs/testing/e2e/manual-test-cases.md` に対応する手動ケース（自動テストで代替できない UI/結合の確認）を追加する。

3. `Skill('steering')`を**振り返りモード**で実行し、`tasklist.md`に申し送り事項を記載する。
  - 実装完了日
  - 計画と実績の差分
  - 学んだこと
  - 次回への改善提案
...
- ステップ8: 追加・変更した自動テストが `docs/testing/` の該当一覧に反映され、`tasklist.md`に申し送り事項が記載されている。
