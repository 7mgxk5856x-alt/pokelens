...
using PokelensMasterDataBuilder.Models;
using PokelensMasterDataBuilder.Pipeline;

// リポジトリルートを特定する。CLAUDE.md の指示通り
// "dotnet run --project tools/PokelensMasterDataBuilder" でリポジトリルートから実行した場合、
// カレントディレクトリがそのままリポジトリルートになる。
if (!Directory.Exists(DataPaths.ToolsDir))
{
    throw new DirectoryNotFoundException(
        $"リポジトリルートが特定できません。CLAUDE.md の指示通り、リポジトリルートから " +
        $"'dotnet run --project tools/PokelensMasterDataBuilder' で実行してください。(CWD: {DataPaths.RepoRoot})");
}
